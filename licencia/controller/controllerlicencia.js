...
$(document).ready(function(){
    ...
    //FUNCION PARA HABILITAR LOS SELECT DE ACUERDO A LA CLASIFICACION Y BRINDAR LA NOMENCLATURA O EL ID
    $("#slclasificacion").change(async function() {
        if($("#slclasificacion").val() == "0")
        {
            //desabilitado
            $('#id_licencia').prop('disabled', true);
        }
        else
        {
            //habilitado
            $('#id_licencia').prop('disabled', false);
...
$("#guardar").click(function()
{
    ...
    if (slrecurrente=="SI" && duracion=="")
    {
        ...
        else if (slrecurrente=="SI" && duracion=="0")
        {
            ...
            {
                guardar(clasificacion,id_licencia,software,dia,mes,anio,duracion,slrecurrente,cantidad,nota);
            }
...
function guardar(clasificacion,id_licencia,software,dia,mes,anio,duracion,slrecurrente,cantidad,nota){
        $.ajax({
            type:"POST",
            url:"consultas.php",
            data:
